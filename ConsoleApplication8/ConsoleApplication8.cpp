...
int main()
{
	...
	HANDLE hConsole = 
	CreateConsoleScreenBuffer(GENERIC_READ | GENERIC_WRITE,  0, NULL, CONSOLE_TEXTMODE_BUFFER, NULL);																	  //handler for console
	...
	while (1){												//game cycle
		...
		if (GetAsyncKeyState((unsigned short)'D') & 0x8000)
			x += (10.0f) * deltaTime;
		screen[int(y) * 120 + int(x)] = '.';											//draw character
		swprintf_s(screen, 40, L"FPS=%3.2f WASD for controls", 1.0f / deltaTime);		//show fps and hint
		WriteConsoleOutputCharacter(hConsole, screen, 3600, { 0, 0 }, &dwBytesWritten); //update all screen
	}
}